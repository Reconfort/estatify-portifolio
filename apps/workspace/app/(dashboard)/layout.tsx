...
/**
 * Workspace shell — grouped sidebar + sticky topbar.
 * Same routes as before; purely a visual upgrade (Linear/Stripe-grade calm).
 */
  ...
  return (
    <>
      ...
      <div className="px-3 pb-2">
        <button
          type="button"
          className="flex w-full items-center gap-2.5 rounded-lg border border-sidebar-border bg-card px-3 py-2.5 text-left shadow-2xs transition-colors hover:bg-sidebar-accent/50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
        >
          <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-brand-700 text-caption font-bold text-neutral-50">
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const role = activeTenant?.role || "Member";

  return (
    <div className="relative flex h-dvh overflow-hidden bg-background text-foreground">
      <DotBackground />
      <div className="relative z-10 flex h-full w-full min-h-0 min-w-0 overflow-hidden">
        ...
        <aside className="hidden h-full w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar md:flex">
          <SidebarContent agencyName={agencyName} role={role} />
        </aside>
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50 flex md:hidden">
            <div
              ...
              aria-hidden
            />
            <aside className="relative flex w-72 max-w-[85vw] flex-col border-r border-sidebar-border bg-sidebar  animate-in slide-in-from-left duration-200">
              <button
                type="button"
                onClick={() => setMobileOpen(false)}
